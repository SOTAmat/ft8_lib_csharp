...
{
    /// <summary>
    /// Contains the LDPC matrix data for FT8/FT4 protocol
    /// </summary>
    internal static class LdpcMatrix
    {
        /// <summary>
        /// Initializes the number of ones per row (Nr) in the LDPC matrix
        /// </summary>
        /// <returns>Array of Nr values</returns>
        internal static int[] GetNr()
        {
            // Corrected values based on C kFTX_LDPC_Num_rows
            return new int[] {
        ...
        /// <summary>
        /// Initializes the number of ones per column (Nc) in the LDPC matrix
        /// </summary>
        /// <returns>Array of Nc values</returns>
        internal static int[] GetNc()
        {
            // This seems correct based on C structure (weight 3 for msg, 4 for parity)
            // Although Mn only stores 3 connections per node.
            int[] nc = new int[Constants.FTX_LDPC_N]; // 174

            // For columns 0-90 (message bits)
            for (int i = 0; i < Constants.FTX_LDPC_K; i++) // 91
            {
                nc[i] = 3;
            }

            // For columns 91-173 (parity bits)
            for (int i = Constants.FTX_LDPC_K; i < Constants.FTX_LDPC_N; i++)
            {
                nc[i] = 4;
            }
        ...
        /// <summary>
        /// Gets the column indices with 1's for each row in the LDPC matrix (Nm)
        /// Ported directly from C kFTX_LDPC_Nm, converting 1-based to 0-based indices.
        /// Padded with -1 where C used 0 for padding.
        /// </summary>
        /// <returns>2D array [83, 7] of column indices for each row</returns>
        internal static int[,] GetNm()
        {
            return new int[Constants.FTX_LDPC_M, 7] {
        ...
        /// <summary>
        /// Gets the row indices with 1's for each column in the LDPC matrix (Mn)
        /// Ported directly from C kFTX_LDPC_Mn, converting 1-based to 0-based indices.
        /// </summary>
        /// <returns>2D array [174, 3] of row indices for each column</returns>
        internal static int[,] GetMn()
        {
            return new int[Constants.FTX_LDPC_N, 3] {
